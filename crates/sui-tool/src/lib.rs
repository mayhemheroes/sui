...
pub async fn state_sync_from_archive_unordered(
    path: &Path,
    genesis: &Path,
    remote_store_config: ObjectStoreConfig,
    concurrency: usize,
    ...
    let state_sync_store = RocksDbStore::new(store, committee_store, checkpoint_store.clone());
    let mut archive_reader =
        ArchiveReaderV1::new(remote_store_config, NonZeroUsize::new(concurrency).unwrap())?;
    archive_reader.sync_manifest_once().await?;
    let latest_checkpoint_in_archive = archive_reader.latest_available_checkpoint().await?;
    info!(
        "Latest available checkpoint in archive store: {}",
    ...
    let counter = Arc::new(AtomicU64::new(latest_checkpoint));
    let cloned_progress_bar = progress_bar.clone();
    let cloned_counter = counter.clone();
    tokio::spawn(async move {
        loop {
            cloned_progress_bar.set_position(cloned_counter.load(Ordering::Relaxed));
            tokio::time::sleep(Duration::from_secs(1)).await;
        }
    });
    let start = latest_checkpoint
    ...
    info!("Starting syncing checkpoints from checkpoint seq num: {start}");
    archive_reader
        .read_unordered(state_sync_store, start..u64::MAX, counter)
        .await?;
    let end = checkpoint_store
        .get_highest_synced_checkpoint_seq_number()?
        .unwrap();
