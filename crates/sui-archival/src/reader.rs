        ...
        <S as ReadStore>::Error: std::error::Error,
    {
        ...
        let results: Vec<Result<(), anyhow::Error>> = futures::stream::iter(files.iter())
            ...
            .map(|(_, (summary_metadata, content_metadata))| {
                ...
                async move {
                    let summary_bytes =
                        Self::download_file(summary_metadata.clone(), remote_object_store.clone())
                            .await?;
                    let content_bytes =
                        Self::download_file(content_metadata.clone(), remote_object_store.clone())
                            .await?;
            ...
            .map_ok(
                |((summary_metadata, summary_bytes), (content_metadata, content_bytes))| {
                    ...
                    let _result: Vec<_> = summary_iter
                        .zip(content_iter)
                        .filter(|(s, _c)| {
                        ...
                        .map(|(summary, contents)| {
                            let verified_checkpoint = Self::insert_checkpoint(&store, summary)?;
                            // Verify content
